...
**authorize @post, :update?**: You can pass a second argument to `authorize` if the name of the permission you
...
**authorize Post**: If you don't have an instance for the first argument to `authorize`, then you can pass
...
```ruby
def show
  @user = authorize User.find(params[:id])  #== @user = User.find(params[:id]); authorize @user
end
```

View: You can easily get a hold of an instance of the policy through the `policy`
...
```
`authorize`: Controller X `policy`: View (+Controller)
...
class PostPolicy < ApplicationPolicy
  ...
  def can_moderate?
    user.admin? || user == record.user || not record.published
  end
end
```
...
- `Scope`: The class has the name `Scope` and is nested under the policy class.
- `user`: The first argument is a user. In your controller, Pundit will call the
  `current_user` method to retrieve what to send into this argument.
- `scope`: The second argument is a scope of some kind on which to perform some kind of
  query. It will usually be an ActiveRecord class or a
  `ActiveRecord::Relation`, but it could be something else entirely.
- `resolve`: Instances of this class respond to the method `resolve`, which should return
...
- `policy_scope`: Controller: `policy_scope(Post)`
- `authorize`: Controller: `authorize Post`
